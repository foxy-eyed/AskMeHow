  ...
  describe 'GET /show' do
    ...
    context 'authorized' do
      ...
      it 'each attachment contains attribute name' do
        expect(response.body).to be_json_eql('fox.jpg'.to_json).at_path('question/attachments/0/name')
      end

      it 'each attachment contains attribute src' do
        expect(response.body).to be_json_eql("/uploads/attachment/file/#{attachment.id}/fox.jpg".to_json)
